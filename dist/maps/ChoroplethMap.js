 ...
 * ```
 */
export function ChoroplethMap({ regions, width, height, visited, ariaLabel, onSelect, selected = null, visitedFill = DEFAULT_VISITED, unvisitedFill = DEFAULT_UNVISITED, }) {
    ...
    // shows.
    const drawn = useMemo(() => [...visited].filter((key) => key in regions).length, [visited, regions]);
    return (_jsx("svg", { viewBox: `0 0 ${width} ${height}`, style: { width: "100%", height: "auto", display: "block" }, 
        ...
        role: "img", "aria-label": ariaLabel(drawn, keys.length), 
        // Tapping the background clears the selection, so there is always a way
        // out without hunting for the selected region again.
        onClick: onSelect ? () => onSelect(null) : undefined, children: ordered.map((key) => {
            const isVisited = visited.has(key);
            const isSelected = key === selected;
            return (_jsx("path", { d: regions[key].path, fill: isVisited ? visitedFill : unvisitedFill, fillOpacity: isSelected ? 0.82 : 1, stroke: isSelected ? SELECTED_STROKE : STROKE, strokeWidth: isSelected ? 0.8 : 0.3, style: { cursor: onSelect ? "pointer" : undefined }, onClick: onSelect
                    ? (e) => {
