 ...
 */
export const STATUS_DOT: Record<Exclude<AppLifecycleStatus, "live">, { color: string; label: string }> = {
 ...
 */
export function appLinksFrom(apps: readonly PlatformApp[] | null | undefined): AppLink[] {
  return (apps ?? [])
    .filter((a): a is PlatformApp & { url: string } => !!a.url)
